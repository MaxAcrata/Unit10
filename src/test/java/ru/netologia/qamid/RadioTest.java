...
public class RadioTest {
   ...
   private Radio radio;
    ...
    @ParameterizedTest
    @CsvSource({
            "0, 1", // Если текущая станция 0, то nextStation 1
            "5, 6", // Если текущая станция 5, то nextStation 6
            "9, 0", // Если текущая станция 9, то nextStation 0
            "-1, 1", // Если текущая станция -1, то nextStation 1 (устанавливается 0, затем увеличивается на 1)
            "10, 0"  // Если текущая станция 10, то nextStation 1 (устанавливается 9, затем увеличивается на 1)
    })
    ...
    @ParameterizedTest
    @CsvSource({
            "0, 9", // Если текущая станция 0, то prevStation 9
            "5, 4", // Если текущая станция 5, то prevStation 4
            "9, 8"  // Если текущая станция 9, то prevStation 8
    })
    public void testPrevStation(int currentStation, int expected) {
        // Устанавливаем начальную станцию
        radio.setCurrentStation(currentStation);

        // Вызываем метод prevStation
        radio.prevStation();
    ...
    @ParameterizedTest
    @CsvSource({
            "0, 1", // Если громкость 0, то после увеличения должна быть 1
            "50, 51", // Если громкость 50, то после увеличения должна быть 51
            "100, 100", // Если громкость 100, то она не должна увеличиваться
            "-1, 1", // Если громкость -1, то устанавливается 0, затем увеличивается на 1
            "101, 100" // Если громкость 101, то устанавливается 100, затем не изменяется
    })
    public void testIncreaseVolume(int currentVolume, int expected) {
        ...
        // Получаем фактическое значение текущей громкости
    ...
    @ParameterizedTest
    @CsvSource({
            "0, 0", // Если громкость 0, то она не должна уменьшаться
